...
Public Class frmWALCUA01
    ...
    Public Structure Ajuste
        Public Clave As String
        Public Monto As Double
    End Structure
    Dim EgresosChe() As Ajuste
    Public Sub New()
    ...
    End Sub
    Private Sub TraeEgresosChe()
        Try
            If EgresosChe.Count = 0 Then
                Exit Sub
            End If
        ...
        Catch ex As Exception
            Exit Sub
        End Try
        For i = 0 To EgresosChe.Count - 1
            If EgresosChe(i).Monto <> 0 Then
                For j = 0 To dtCUA.Count - 1
                    filaCUA = dtCUA.Item(j)
                    If filaCUA.CTE_CODIGO = EgresosChe(i).Clave Then
                        filaCUA.CUA_EGRESOSCHE = EgresosChe(i).Monto
                    End If
                Next
            End If
        Next
    ...
    Private Sub TotalesH()
        ...
        For i = 0 To dtCUA.Count - 1
            ...
            End If
            If Not filaCUA.IsCUA_EGRESOSCHENull Then
                dblEgresosChe = filaCUA.CUA_EGRESOSCHE
            End If
            ...
            filaCUA.CUA_PROXIMOSALDO = filaCUA.CUA_SALDO2 - dblEgresosChe
            taCUA.Update(dtCUA)
        Next
    End Sub
    ...
    Private Sub GridGroupingControl1_TableControlCellMouseUp(sender As Object, e As GridTableControlCellMouseEventArgs) Handles GridGroupingControl1.TableControlCellMouseUp
        ...
        Else
            ...
            Me.TraeCuadraturaBindingSource.EndEdit()
            DsWALCUA.TraeCuadratura.AcceptChanges()

            ' Modifcacoón para Capturar los Egresos****************************************************************************
            Dim filaTraeCua As DataRowView

            ReDim EgresosChe(TraeCuadraturaBindingSource.Count - 1)
            For i = 0 To TraeCuadraturaBindingSource.Count - 1
                filaTraeCua = TraeCuadraturaBindingSource.Item(i)
                If IsNumeric(filaTraeCua.Item("CUA_EGRESOSCHE")) Then
                    'filaCUA = dtCUA.FindByCUA_CODIGO(filaTraeCua.CUA_CODIGO)
                    EgresosChe(i).Monto = filaTraeCua.Item("CUA_EGRESOSCHE")
                    EgresosChe(i).Clave = filaTraeCua.Item("CTE_CODIGO")
                End If
            Next
